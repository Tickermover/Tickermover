"""
AlphaHunt - AI AlphaHunt Score Engine
...
def _add_signals(t: dict, breakdown: dict, signals: list) -> None:
    ...
    if sv is not None and sv >= 3:    signals.append(f"AlphaHunt Score climbing +{sv:.1f} pts in 24h")
    elif sv is not None and sv <= -3: signals.append(f"AlphaHunt Score falling {sv:.1f} pts in 24h")
