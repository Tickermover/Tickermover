...
Houses the long-form HTML renderers for:
  ...
  • /sectors/{slug}            — one page per sub-sector with live Pop Scores
...
def newsletter_block(source: str) -> str:
    ...
    return f"""
<div class="nl">
  <h3>Get the weekly Pop Score digest</h3>
...
<div class="cta">
  ...
  <p>200+ US stocks. Pop Score, conflict detection, Reverse DCF, peer comparison. We do the homework.</p>
...
PILLARS: dict[str, dict] = {
    "pop-score": {
        "title": "What is Pop Score? AlphaHunt's 0-100 stock rating explained",
        "desc": "Pop Score is AlphaHunt's 0-100 composite rating that blends fundamentals, momentum, valuation, analyst signal, and macro regime into a single plain-English verdict.",
        "h1": "What is the Pop Score?",
...
<p>Every Pop Score is built from five weighted components. Each one is normalized to 0-100 inside its own peer group, so a small-cap quantum stock and a mega-cap chipmaker are graded against their own kind.</p>
<ul>
...
<blockquote>The Pop Score isn't a buy signal. It's a "homework checklist completed" signal. It tells you the stock's quantitative story is good. The qualitative judgment — does the thesis make sense to <em>you</em>? — is still yours.</blockquote>
<ul>
...
<h2>Smart Score vs raw Pop Score</h2>
<p>You'll sometimes see two numbers — a raw Pop Score and a Smart Score. The Smart Score is the same composite, but adjusted for the current market regime. In a bullish regime, the Smart Score tilts toward growth and momentum. In a bearish regime, it tilts toward quality, balance-sheet strength, and valuation discipline. The raw Pop Score is regime-blind; the Smart Score adapts.</p>

<h2>The conflict flag</h2>
<p>Numbers can lie when they're averaged. A stock can earn a 78 Pop Score because four components are strong — but if the fifth is screaming "danger" (insider selling spike, EPS estimate crash, margin collapse), AlphaHunt flags it with a <code>caution</code> badge and rewrites the bottom-line verdict accordingly. <a href="/learn/how-to-read-fundamentals">More on how we read each signal here.</a></p>

<h2>What it doesn't capture</h2>
<p>Pop Score is a quantitative framework. It cannot price in: regulatory risk, executive turnover, accounting irregularities, fraud, geopolitical exposure, or anything that isn't in the public filings yet. Treat it as the starting point of your research, not the conclusion.</p>

{cta_block("See live Pop Scores")}
{newsletter_block("learn-pop-score")}
<div class="legal">AlphaHunt is a research tool, not financial advice. Pop Score is a composite signal — always do your own research before investing.</div>
"""
...
<h2>What it doesn't capture</h2>
<p>Reverse DCF assumes margins, tax rates, and capital intensity stay roughly constant. For companies undergoing big margin shifts (early-stage SaaS scaling to profitability, hardware companies losing pricing power), the implied growth read can be misleading. Use it together with the Pop Score's <a href="/learn/pop-score">Fundamentals component</a>, not in isolation.</p>
...
<p>Every stock in our universe is run through this checklist every five minutes during market hours. The output is the <a href="/learn/pop-score">Pop Score</a> — a single 0-100 composite that bakes in all seven signals plus a regime adjustment. You can drill into the underlying components on any stock's detail page.</p>
...
<div class="wrap">
  ...
  <h1>Learn</h1>
  <p class="lede">Plain-English guides to the methodology behind AlphaHunt — the Pop Score, the Reverse DCF, and how to read US stock fundamentals.</p>
  ...
  <div class="legal">AlphaHunt — research, not advice.</div>
    ...
    return page_shell(
        title="Learn — AlphaHunt methodology, Pop Score, Reverse DCF",
        desc="Plain-English guides to AlphaHunt's stock-research methodology — Pop Score, Reverse DCF, and the seven-step fundamentals checklist.",
...
def render_sector(slug: str, universe: list[dict], site_origin: str) -> Optional[str]:
    ...
    title = f"Best {label} stocks — live Pop Scores | AlphaHunt"
    desc = (
        f"AlphaHunt's live ranking of {n} {label} stocks by Pop Score. "
...
<div class="wrap-wide">
  ...
  <p class="lede">Live ranking of {n} {label} stocks by AlphaHunt Pop Score — a 0-100 composite of fundamentals, valuation, momentum, analyst signal, and macro regime. Click any ticker for the full breakdown.</p>
  <table class="tbl">
    <thead><tr><th>Ticker</th><th>Grade</th><th>Pop</th><th>Bottom line</th></tr></thead>
    <tbody>{table_html or '<tr><td colspan="4">No stocks scored in this sector yet — the universe is warming up.</td></tr>'}</tbody>
  </table>
  <p style="font-size:13px;color:#64748b">Pop Scores update every 5 minutes during US market hours. Grades: <strong>A</strong> Strong Buy · <strong>B</strong> Buy · <strong>C</strong> Hold · <strong>D</strong> Reduce · <strong>F</strong> Avoid.</p>
...
def render_sector_index(universe: list[dict], site_origin: str) -> str:
    ...
    cards = "".join(
        ...
        f'<div class="sub">View live Pop Scores for stocks in this sector.</div></a></div>'
...
<div class="wrap">
  ...
  <h1>Sectors</h1>
  <p class="lede">{len(smap)} sub-sectors covered, all scored on the same 0-100 Pop Score. Pick one to see a live ranking.</p>
  <div class="cards">{cards}</div>
  ...
  <div class="legal">AlphaHunt — research, not advice.</div>
    ...
    return page_shell(
        title="Stock sectors — live Pop Scores by sub-sector | AlphaHunt",
        desc="Browse AlphaHunt's stock universe by sub-sector — AI semiconductors, cybersecurity, quantum computing, photonics, and more. Live Pop Scores updated every 5 minutes.",
        canonical=canonical, body_html=body,
        og_image=f"{site_origin}/static/icons/icon-512.png",
    )
...
<div class="cmp-card">
  ...
  <div class="cmp-row"><span class="k">Pop Score</span><span class="v">{pop_n}/100</span></div>
...
def render_comparison(a: str, b: str, universe: list[dict], site_origin: str) -> Optional[str]:
    ...
    # Compute the verdict — which one wins on Pop Score
    ...
    try:
        ...
        pa_f, pb_f = 0.0, 0.0
    if abs(pa_f - pb_f) < 3:
        verdict = f"{a} and {b} score within 3 points of each other on AlphaHunt's Pop Score — effectively tied. The right pick depends on which thesis you find more compelling."
    elif pa_f > pb_f:
        verdict = f"{a} edges out {b} on AlphaHunt's composite Pop Score ({round(pa_f)} vs {round(pb_f)}). The breakdown below shows where each stock leads."
    else:
        verdict = f"{b} edges out {a} on AlphaHunt's composite Pop Score ({round(pb_f)} vs {round(pa_f)}). The breakdown below shows where each stock leads."
    title = f"{a} vs {b} — Pop Score, valuation, growth compared | AlphaHunt"
    desc = (
        f"Side-by-side comparison of {a} ({name_a[:24]}) and {b} ({name_b[:24]}) — "
        f"Pop Score, growth, valuation, momentum and analyst upside. Updated every 5 minutes."
    )[:160]
    body = f"""
<div class="wrap-wide">
  {brand_header()}
  ...
  <p>The Pop Score blends fundamentals, valuation, momentum, analyst signal and macro regime into one 0-100 number. <a href="/learn/pop-score">Read the methodology →</a></p>
...
def render_compare_index(universe: list[dict], site_origin: str) -> str:
    ...
    cards = "".join(
        ...
        f'<div class="sub">Side-by-side Pop Score, growth, valuation and momentum.</div></a></div>'
        for a, b in valid
    )
    body = f"""
<div class="wrap">
  ...
  <p class="lede">Curated head-to-head pages for the most-asked-about US stocks. Pop Score, growth, valuation and analyst signal — all on one page. You can also build any comparison by visiting <code>/compare/&lt;TICKER1&gt;-vs-&lt;TICKER2&gt;</code>.</p>
  <div class="cards">{cards}</div>
  ...
  <div class="legal">AlphaHunt — research, not advice.</div>
    ...
    return page_shell(
        ...
        desc="Side-by-side US stock comparisons — Pop Score, growth, valuation, analyst upside. Curated head-to-head pages updated every 5 minutes.",
        canonical=canonical, body_html=body,
        og_image=f"{site_origin}/static/icons/icon-512.png",
    )
