...
async def report_page(ticker: str):
    ...
    inbound links and accumulated search equity carry over. The old Reading Mode
    renderer was deleted on 14 Aug 2026; recover it from git history if that
    design is ever wanted again."""
    from fastapi.responses import RedirectResponse
    sym = ticker.upper().strip()
    return RedirectResponse(url=f"/stocks/{sym}", status_code=301)


# The Reading Mode renderer was DELETED here on 14 Aug 2026 (377 lines).
# The legacy /report/{ticker} route above has 301'd to /stocks/{ticker} since
# 1 Aug and nothing else ever called it, so it was unreachable code.
# Keeping it 'for reference' turned it into a trap: it still read like the live
# per-stock page, so an edit meant for /report landed in code that can never
# render. /stocks/{ticker} -> _render_stock_page() is the only per-stock page.
...
def _unsub_secret() -> str:
    return (config.SUPABASE_SERVICE_KEY or SITE_ORIGIN or "tm-unsub")[:64]
