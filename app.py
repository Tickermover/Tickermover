...
def _build_landing_schema() -> str:
    ...
    app_schema = {
        ...
        "operatingSystem": "Web",
        "description": "Real-time US stock research with Pop Score, plain-English verdicts, conflict detection, Reverse DCF, and macro-aware scoring. Free during beta.",
        "offers": {
            "@type": "Offer",
            "price": "0",
            "priceCurrency": "USD",
...
async def stock_page(ticker: str):
    ...
    Each page targets long-tail searches like "NVDA pop score",
    "NVDA stock analysis", "should I buy NVDA". Includes:
      ...
      - Pop Score, verdict, valuation, news inline (visible to crawlers)
...
def _render_stock_page(t: dict) -> str:
    ...
    bottom_line = t.get("bottom_line") or f"{name} is currently scored {round(pop)}/100 on AlphaHunt's Pop Score."
    ...
    title = f"{sym} Stock Analysis · Pop Score {round(pop)} · {rating} | AlphaHunt"
    desc  = (
        f"{name} ({sym}) — current Pop Score {round(pop)}/100 ({rating}). "
        f"{bottom_line[:140]}"
    )
...
<div class="wrap">
  ...
  <div class="verdict-box">
    <div class="verdict-head">
      ...
      <span class="verdict-score">{round(pop)}<span class="lbl">/ Pop Score</span></span>
    </div>
  ...
  <h2>Key metrics</h2>
  <div class="metrics">
    <div class="metric"><div class="lbl">Pop Score</div><div class="val">{round(pop)}/100</div></div>
  ...
  <div class="legal">
    AlphaHunt is a research tool, not financial advice. Pop Score is a composite signal — always do your own research before investing.
...
async def learn_pillar(slug: str):
    """Evergreen explainer pages — Pop Score, Reverse DCF, fundamentals."""
...
async def sector_page(slug: str):
    """One landing page per sub-sector with live Pop Scores."""
...
def _render_og_png(t: dict) -> bytes:
    ...
    grade  = t.get("grade") or "—"
    rating = {"A":"STRONG BUY","B":"BUY","C":"HOLD","D":"REDUCE","F":"AVOID"}.get(grade, "UNDER REVIEW")
    bl = (t.get("bottom_line") or f"{name} scored {pop_n}/100 on AlphaHunt's Pop Score.")[:120]
    ...
    draw.text((cx - 60, cy + 70), "POP SCORE", font=f_lbl, fill=(148, 163, 184))
...
<h3>Technical Setup</h3>
<p>AAOI is one of the strongest momentum names in our universe, with a Pop Score of <strong>75</strong> and RS Rating of 75. The stock has gained +18.4% in today's session on volume 2.7x the 20-day average — institutional accumulation signal confirmed. EPS has beaten estimates 4 consecutive quarters with expanding gross margins, confirming the revenue growth is translating to the bottom line.</p>
...
<h3>AlphaHunt View</h3>
<p>AAOI earns a <strong>STRONG BUY</strong> with 75 Pop Score. The 800G/1.6T upgrade cycle is a multi-year tailwind with AAOI positioned as a beneficiary. Analyst mean target implies <strong>+41.7% upside</strong> from current levels.</p>"""
  },
  {
    "id": "mu-hbm-ai-2026",
...
<h3>Technical Setup</h3>
<p>MU carries a Pop Score of <strong>76</strong> and RS Rating of 73. Today's -2.18% session is noise against a backdrop of +30.6% gains over the past month. RSI at 64 remains healthy — not overbought. Social mention velocity +231% vs 24h confirms the stock is on investors' radar.</p>
...
<h3>Technical Setup</h3>
<p>GSAT carries a Pop Score of <strong>72</strong> with RS Rating 81 — the highest RS in this analysis. Breaking to a new 52-week high confirms institutional accumulation. The stock has gained +35.2% over the past month. EPS beat 4 consecutive quarters. Analyst mean target of <strong>$85 (+5.6% upside)</strong> is conservative given the optionality of the Apple partnership.</p>
...
<h3>Technical Setup</h3>
<p>CRDO is the top performer in our Model Portfolio, with Pop Score of <strong>71</strong> and one of the strongest momentum profiles in the AlphaHunt universe. RSI at 64 — ideal momentum zone — suggests room to run without overextension. High-speed SerDes is a winner-take-most market, and Credo has won.</p>
...
<h3>Technical Setup</h3>
<p>SOUN carries a Pop Score of <strong>75</strong> with RS Rating 72 and an analyst mean price target of <strong>$14.63 (+78.4% upside)</strong>. Revenue growing at +59.4% YoY. Short interest at 36.1% creates significant squeeze potential. EPS beat 4 consecutive quarters.</p>
...
<h3>Technical Setup</h3>
<p>NVT has a Pop Score of <strong>75</strong> and RS Rating 67, within 1.3% of its 52-week high — a classic institutional accumulation pattern. EPS beat 4 of 4 last quarters. Gross margin expanding confirms operating leverage. No overbought RSI concern.</p>
...
<h3>Technical Setup</h3>
<p>IONQ carries a Pop Score of <strong>72</strong> with analyst mean target of <strong>$17.83 (+96.9% upside)</strong>. Within 22.4% of 52-week high after consolidation. EPS beat 4 consecutive quarters with expanding margins. Social momentum building as quantum milestones approach.</p>
...
<h3>Technical Setup</h3>
<p>WDC has a Pop Score of <strong>74</strong> with momentum confirming from the HDD cycle bottom. EPS beat 4 of 4 last quarters with gross margin expansion. High-density PDUs and thermal management for AI DCs driving incremental demand.</p>
...
<h3>Technical Setup</h3>
<p>UNIT carries a Pop Score of <strong>71</strong> with RS Rating 71. The stock's consistent momentum and fiber demand tailwind make it one of the more stable high-Pop names in our universe. RSI at 52 — ample room to run.</p>
...
<h3>Technical Setup</h3>
<p>LSCC carries a Pop Score of <strong>74</strong> with RS Rating 71 and revenue growing +24.2% YoY. EPS beat 4 consecutive quarters with gross margins expanding. P/E at 5779× is extreme — but reflects near-zero current earnings during an investment phase, not permanent multiple expansion. Analyst mean target: <strong>$143 (+22% upside)</strong>.</p>
...
<p>With CRDO up 87.7% and UNIT up 52.2%, consider partial profit-taking to manage position sizing. The Model Portfolio's 30-day inception window suggests rebalancing in two weeks. Names with Pop Scores that have declined since entry (check All Stocks tab for current scores) may be candidates for rotation.</p>
...
def _is_hot_eligible(t: dict) -> bool:
    ...
    1. POP score  ≥ 70  (Grade A territory — top-tier momentum + fundamentals)
...
def _build_model_portfolio() -> dict:
    """
    Select top 20 Grade-A stocks by Pop Score.
